...
import type { Artist, Category } from "@/lib/supabase";

function Label({ children, required }: { children: React.ReactNode; required?: boolean }) {
  return (
    <label style={{ display: "block", fontSize: "0.7rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--text-3)", marginBottom: "0.5rem" }}>
...
export default function EditArtistPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [form, setForm] = useState({
    ...
    image_url: "", notable_works: "", artwork_images: [] as string[],
  });
  ...
  async function init(artistSlug: string) {
    ...
    const [{ data: artistData, error: fetchErr }, { data: catData }] = await Promise.all([
      supabase.from("artists").select("*").eq("slug", artistSlug).single(),
      supabase.from("categories").select("*").order("name"),
    ]);

    if (fetchErr || !artistData) { setError("Artist not found."); setLoading(false); return; }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const { error: saveErr } = await supabase.from("artists").update({
      name: form.name.trim(),
    ...
    else { setSuccess(true); setTimeout(() => router.push(`/artists/${slug}`), 1200); }
  }
    ...
    <div style={{ minHeight: "100vh" }}>
      ...
      <div style={{ padding: "2rem 1.25rem 1.5rem", borderBottom: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          <Link href={`/artists/${slug}`} style={{ color: "var(--text-3)", fontSize: "0.82rem", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: "0.4rem", marginBottom: "1rem" }}>
            ← Back to profile
          </Link>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.75rem" }}>
            <div>
              <p style={{ color: "var(--orange)", fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: "0.25rem" }}>Admin · Edit</p>
              <h1 style={{ color: "var(--text-1)", fontFamily: "Georgia, serif", fontSize: "clamp(1.4rem, 3vw, 2rem)", fontWeight: 700, letterSpacing: "-0.02em" }}>{artist.name}</h1>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 760, margin: "2rem auto 0", padding: "0 1.25rem 5rem" }}>
        {success && (
          <div style={{ background: "rgba(122,155,106,0.12)", border: "1px solid rgba(122,155,106,0.3)", borderRadius: 12, padding: "0.9rem 1.1rem", color: "#a0c890", fontSize: "0.88rem", marginBottom: "1.5rem" }}>
            ✓ Saved! Redirecting…
          </div>
        )}
        {error && (
          <div style={{ background: "rgba(232,98,58,0.08)", border: "1px solid rgba(232,98,58,0.2)", borderRadius: 12, padding: "0.9rem 1.1rem", color: "#f0a07a", fontSize: "0.88rem", marginBottom: "1.5rem" }}>
            {error}
          </div>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: "1rem", alignItems: "flex-start", flexWrap: "wrap" }}>
                <div style={{ width: 100, height: 130, borderRadius: 12, overflow: "hidden", border: "1px solid var(--border)", background: "var(--surface-2)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {form.image_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={form.image_url} alt="Portrait" referrerPolicy="no-referrer" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                  ) : (
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "0.6rem", minWidth: 200 }}>
                  ...
                  <button type="button" className="btn-ghost" onClick={() => portraitInputRef.current?.click()} disabled={uploadingPortrait} style={{ width: "100%", justifyContent: "center" }}>
                    {uploadingPortrait ? "Uploading…" : "↑ Upload image"}
                  </button>
                  <input className="input-dark" type="url" placeholder="Or paste image URL…" value={form.image_url} onChange={set("image_url")} />
                </div>
              </div>
            </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "0.75rem", marginBottom: "1rem" }}>
                  {form.artwork_images.map((url, i) => (
                    <div key={i} style={{ position: "relative", aspectRatio: "4/3", borderRadius: 10, overflow: "hidden", border: "1px solid var(--border)", background: "var(--surface-2)" }}>
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={url} alt={`Artwork ${i + 1}`} referrerPolicy="no-referrer" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                      <button type="button" onClick={() => removeArtwork(i)}
                        style={{ position: "absolute", top: 6, right: 6, background: "rgba(0,0,0,0.7)", border: "none", borderRadius: "50%", width: 24, height: 24, color: "#fff", fontSize: "0.75rem", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1 }}
                        title="Remove">✕</button>
                    </div>
                  ))}
                </div>
              )}
              <input ref={artworkInputRef} type="file" accept="image/*" style={{ display: "none" }}
                onChange={e => { const f = e.target.files?.[0]; if (f) uploadArtwork(f); }} />
              <button type="button" className="btn-ghost" onClick={() => artworkInputRef.current?.click()} disabled={uploadingArtwork} style={{ width: "100%", justifyContent: "center" }}>
                {uploadingArtwork ? "Uploading…" : "↑ Upload artwork image"}
              </button>
            </div>

            <div style={{ display: "flex", gap: "0.75rem", paddingTop: "0.5rem" }}>
              <button type="submit" className="btn-primary" disabled={saving || success} style={{ padding: "0.7rem 2rem", fontSize: "0.92rem", opacity: saving ? 0.7 : 1 }}>
                {saving ? "Saving…" : "Save Changes"}
              </button>
              <Link href={`/artists/${slug}`} className="btn-ghost" style={{ padding: "0.7rem 1.25rem" }}>Cancel</Link>
            </div>
